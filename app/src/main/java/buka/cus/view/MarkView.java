...
public class MarkView extends View {
    ...
    public MarkView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        DEFAULT_TEXT_SIZE = UtilBuEks.spToPx(getResources(), 16);
        MIN_SIZE = (int) UtilBuEks.dpToPx(getResources(), 48);
        DEFAULT_STROKE_WIDTH = UtilBuEks.dpToPx(getResources(), 2);
        DEFAULT_BG_RING_WIDTH = UtilBuEks.dpToPx(getResources(), 2);
    ...
    private void initByAttributes(TypedArray attributes) {
        ...
        if (!isInEditMode()) {
            ...
            mStrokeColors = getResources().getIntArray(id);
        }
...
//        String text = isMarkValid(mMark) ? ( String.valueOf(mMark) + "/" + String.valueOf(mMax) ) : "-";
        String text = generateInnerText(mMark, mMax);
    ...
    private String generateInnerText(final int mark, final int max) {
        if (mark <= 0)
            return "0/" + max;
        if (mark >= max)
            return "✓";
        if (isMarkValid(mark))
            return String.valueOf(mark) + "/" + String.valueOf(max);
        return "-";
    }
...
//        return mStrokeColors[mMark - 1];
        return mStrokeColors[getMarkPos() - 1];
    }

    private int getMarkPos() {
        try {
            return (mMark * mStrokeColors.length) / mMax;
//            double res = (Double.valueOf(String.valueOf(mMark)) / Double.valueOf(String.valueOf(mMax))) * Double.valueOf(String.valueOf(mStrokeColors.length));
//            res = Math.round(res);
//            return Double.valueOf(res).intValue();
        } catch (Exception e) {
//            Log.e("MARK VIEW", e.getMessage());
            return 1;
        }
    }
